...
变体："""
    ...
    def _record_stage_usage(
        ...
        *,
        ...
        trace.stage_token_usage[name] = TokenUsage(
            ...
            reranker_tokens=max(0, int(reranker_tokens)),
            total_tokens=total_tokens + max(0, int(reranker_tokens)),
        )
    ...
    def prepare(
        ...
        *,
        ...
        candidates = self._adaptive_retrieve(
            ...
            trace,
            request_started,
            allow_multiquery=allow_multiquery,
        )
    ...
    def invoke(
        ...
        stage_started = time.perf_counter()
        citations = self._filter_used_citations(answer, citations)
        self._thread_local.last_citations = citations
        verification = None
        verification_usage_before = self._usage_snapshot(self.llm)
        if self.citation_verifier:
            ...
            answer = self.citation_verifier.apply_policy(answer, verification)
        self._record_stage_usage(
            trace,
            "citation_verification",
            verification_usage_before,
            self._usage_snapshot(self.llm),
        )
        ...
        self._record_span(
            trace, "citation_verification", request_started, stage_started,
            {
                "status": verification.status if verification else "disabled",
                **self._latest_component_event(
    ...
    def stream(
        ...
        citations = self._filter_used_citations(generated_answer, citations)
        verification = None
        ...
        if self.citation_verifier:
            verification = self.citation_verifier.verify(
                generated_answer, citations, status,
                timeout=self.citation_verification_timeout_seconds,
            )
            final_answer = self.citation_verifier.apply_policy(generated_answer, verification)
        self._record_stage_usage(
            trace,
            "citation_verification",
            verification_usage_before,
        ...
        self._record_span(
            trace, "citation_verification", stream_started, verification_started,
            {
                "status": verification.status if verification else "disabled",
                **self._latest_component_event(
                    self.llm, "citation_verification"
